  ...
  return (
    <header className="sticky top-0 z-40 bg-[#0A0A0A]/90 backdrop-blur-xl border-b border-rose-900/30">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
        ...
        <nav className="hidden lg:flex items-center gap-4 text-xs font-bold uppercase tracking-wider overflow-x-auto no-scrollbar [ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden py-2">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className={`transition-colors whitespace-nowrap py-1 ${pathname === link.href ? 'text-rose-500 border-b-2 border-rose-500 font-bold' : 'text-slate-400 hover:text-white'}`}
            >
              {link.label}
            </Link>
          ))}
  ...
  return (
    <footer className="bg-[#050505] border-t border-rose-900/30 py-12 text-slate-400 text-xs">
      ...
      <div className="max-w-7xl mx-auto px-4 mt-8 pt-6 border-t border-slate-900 text-center text-slate-500 text-[11px]">
        © 2026 IronCore Fitness Group. Unforgiving Performance.
      </div>
